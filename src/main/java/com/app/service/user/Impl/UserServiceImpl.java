...
//		// id pw 일치하는가?
//
//		//케이스1) DB에서 정보를 조회 한 후 -> Service Layer 상태에서 비교하는 로직을 수행
//		
...
//		return null;
		...
		/*
		 * return 의미가 담긴 코드 (SUC, FAL, LCK)... int 숫자 return -> 1:성공 2:id는 맞는데 비번이 틀렸다
		 * 3:아이디없다 4:신고로잠겼다 5: 휴면계정 6:...
		 */
		// 케이스 2) DB에서 쿼리를 통해, 정상여부 체크 로직 수행
		// userDAO.checkUserLogin -> sql query상에서 id,pw,userType 동일한 경우를 체크
	...
	public boolean isDuplicatedId(String id) {
		...
		if(user == null) { //해당 아이디 객체가 없다 -> id값 사용안함, -> 중복 X
			return false;
		}else {  //해당 아이디가 있다면 -> id값이 pk , 사용중, -> 중복 O
			return true;
		}	
	}
