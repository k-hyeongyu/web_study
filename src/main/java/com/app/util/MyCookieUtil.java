...
public class MyCookieUtil {
	...
	/*
	 * // 전부 중복 작성 - 리팩토링 Before public static String getCookie(HttpServletRequest
	 * request, String cookieName) { Cookie[] cookies = request.getCookies();
	 * 
	 * //cookies 뭉치가 여기 있으니 이중에서 cookieName의 value를 찾아서 반환해라 try { for(Cookie ck :
	 * cookies) {
	 * 
	 * if(ck.getName().equals(cookieName)) { String cookieValue =
	 * URLDecoder.decode(ck.getValue(), "UTF-8"); return cookieValue; } } } catch
