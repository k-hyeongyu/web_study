...
public class Scope01Controller {
	...
	public String scope2(HttpServletRequest request) {
		...
		// session scope
		request.setAttribute("requestMsg", "이것은 model에 추가한 메시지(request scope)");
	...
	public String scope4(HttpSession session) {
		...
		session.removeAttribute("sessionMsg"); //session scope에서 특정 키값을 삭제
		
		session.invalidate(); //session scope에서 저장된 모든 속성(키값) 삭제 (세션 초기화)
