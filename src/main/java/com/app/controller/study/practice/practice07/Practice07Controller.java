...
public class Practice07Controller {
	...
	public String listTest(@RequestParam String type, Model model) {
		...
		/*
		 * //type : str 혹은 member 둘중에 하나로 확정 전제 if(type.equals("str")) {
		 * 
		 * model.addAttribute("msg", "스트링 리스트입니다.");
		 * 
		 * List<String> msgList = new ArrayList<String>(); for(int i=1; i<=10; i++) {
		 ...
		 * 
		 * for(int i=1; i<=5; i++) { memberList.add( new Member("아이디"+i, "비번"+i, "이름"+i)
		 * ); }
		 * 
		 * 
		 * // memberList.add( new Member("아이디1", "비번1", "이름1") ); // memberList.add( new
		 * Member("아이디2", "비번2", "이름2") ); // memberList.add( new Member("아이디3", "비번3",
		 * "이름3") ); // memberList.add( new Member("아이디4", "비번4", "이름4") ); //
